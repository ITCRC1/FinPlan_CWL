...
/** ⚠️ **Esto NO es una `page.tsx`, y no puede volver a serlo.**
 *
 * Vivia en `pl-detail/page.tsx` y ahi recibia `esPre` como prop. Next valida
 * que las props de una ruta sean `PageProps` —`params` / `searchParams`— y el
 * build fallo con:
 *
 *     Type '{ esPre?: boolean | undefined } | undefined' does not satisfy
 *     the constraint 'PageProps'.
 *
 * ⚠️ **`tsc --noEmit` NO lo atrapa.** Esa restriccion la generan los tipos de
 * ruta que Next escribe durante `next build`, asi que el typecheck pasaba
 * limpio y el deploy se caia. Una pantalla que recibe props va en su propio
 * archivo y la ruta la envuelve.
 */
export default function PLDetail({ esPre = false }: { esPre?: boolean }) {
