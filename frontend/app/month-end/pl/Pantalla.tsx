...
const VISTAS = [
  ...
  { key: "planillaCuentas" },
  // Owner, 2026-09-10: «un tab adicional […] departamento, cuenta y posición
  // — la posición en las cuentas 6 es el tercer nivel», «eso solo para
  // actuales del mes». Es el mismo dinero de `planillaCuentas`, abierto por
...
/** Los dos modos en que se usa esta pantalla.
 *
 * `cierre` — Cierre de Mes → P&L. La de siempre: tres horizontes (mes, YTD,
 * año), cuatro ranuras libres, y lee los escenarios reales.
 *
 * `pre-cierre` — Pre-Closing. Owner, 2026-09-10: *«el objetivo es revisar solo
 * mes rapido versus budget y forecast principalmente, pero quizas mismo mes
 * año pasado sea tambien correcto»*. Mismos 19 sub-tabs, ninguno se quita —
 * son el objeto del analisis. Cambian tres cosas:
 *
 *   · el horizonte queda FIJO en mes y el selector no se dibuja;
 *   · las ranuras arrancan en Espejo · Forecast · Budget · Actual del año
 *     pasado, que es la comparacion que se pidio;
 *   · la lista de escenarios incluye el espejo del Pre-Cierre, que en el resto
 *     de la app es invisible (ver migracion 140).
 */
export type ModoPL = "cierre" | "pre-cierre";

export default function MonthEndPLPage({ modo = "cierre" }: { modo?: ModoPL }) {
  const esPre = modo === "pre-cierre";
  ...
  useEffect(() => {
    try {
      ...
      // En Pre-Closing el horizonte es mes y punto: restaurar un YTD guardado
      // dejaria la pantalla mostrando un acumulado en la vista que existe para
      // mirar UN mes.
      if (!esPre && (g.horizonte === "month" || g.horizonte === "ytd"
                     || g.horizonte === "full")) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const all = await getScenarios(HOTEL_ID, esPre);
        setEscenarios(all);
  ...
  return (
    <div className="pag pag-ancha" style={{ padding: "18px 22px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 10 }}>
        ...
        {/* ⚠️ En Pre-Closing no hay selector: el horizonte es el MES. La
            pantalla existe para «revisar solo mes rapido versus budget y
            forecast», y un YTD ahi responde otra pregunta. El acumulado sigue
            estando en Cierre de Mes, donde es el trabajo. */}
        {!esPre && (["month", "ytd", "full"] as const).map(k => (
      ...
      {vista === "planillaPosicion" && (() => {
         ...
         *
         * ⚠️ Sale del BORRADOR del Pre-Cierre, no de un escenario. Es lo que
         * significa «los actuales del mes»: lo último que se subió, sin pasar
         * a Final. Por eso ignora las ranuras de comparación — no hay contra
         * qué comparar: un presupuesto no se planea por posición.
         *
         * ⚠️ Y NO es un total nuevo: suma exactamente la planilla del nivel
         * cuenta. El TOTAL viene del backend por eso mismo. */
        const d = planillaPos;
        if (!d) return (
          <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
            {tc("loading")}
        ...
        /* Agrupado por departamento, y dentro por cuenta: es como se lee un
         * reporte de planilla, y es lo que hace legible un cuadro de 244
         * filas — «un reporte grande porque desgrana toda la cuenta y
         * departamento». */
        const porDepto: { code: string; nombre: string; total: number;
                          cuentas: { cuenta: string; nombre: string; total: number;
                                     filas: typeof d.filas }[] }[] = [];
        for (const f of d.filas) {
          let dep = porDepto.find(x => x.code === f.dept_code);
          if (!dep) {
            dep = { code: f.dept_code, nombre: f.dept_name, total: 0, cuentas: [] };
            porDepto.push(dep);
          }
          dep.total += f.monto;
          let cta = dep.cuentas.find(x => x.cuenta === f.cuenta);
          if (!cta) {
            cta = { cuenta: f.cuenta, nombre: f.cuenta_nombre, total: 0, filas: [] };
            dep.cuentas.push(cta);
          }
          cta.total += f.monto;
          cta.filas.push(f);
        }
